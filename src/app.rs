use crate::alarm::{Alarm, AlarmConfig, AudioCommand};
        ...
        ui.add_enabled_ui(!self.countdown.running(), |ui| {
            ui.horizontal_wrapped(|ui| {
                ...
                for (index, suffix, maximum) in
                    [(0, " h", 99_u64), (1, " min", 59_u64), (2, " s", 59_u64)]
                {
                    ui.add(
    ...
    fn compact_panel(&mut self, ctx: &egui::Context) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            ui.horizontal_centered(|ui| {
                ui.label(
                    RichText::new(if self.mode == Mode::Stopwatch {
                        "⏱"
                    } else {
                        "⏳"
                    })
    ...
    fn lap_list(&mut self, ui: &mut egui::Ui, ctx: &egui::Context) {
        ...
        ui.horizontal_wrapped(|ui| {
            ...
            if ui
                .add_enabled(
                    !self.laps.is_empty(),
                    egui::Button::new(language.tr("copy_csv")),
                )
                .clicked()
            {
                self.copy_laps(ctx);
            ...
            if ui
                .add_enabled(
                    !self.laps.is_empty(),
                    egui::Button::new(language.tr("export_csv")),
                )
                .clicked()
            {
    ...
    fn update(&mut self, ctx: &egui::Context, _: &mut eframe::Frame) {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().id_salt("main").show(ui, |ui| {
                ...
                ui.horizontal_wrapped(|ui| {
                    ui.selectable_value(&mut self.mode, Mode::Stopwatch, language.tr("stopwatch"));
                    ui.selectable_value(&mut self.mode, Mode::Countdown, language.tr("countdown"));
                ...
                if self.finished {
                    ui.horizontal_wrapped(|ui| {
                        ui.label(RichText::new(language.tr("time_up")).strong().color(ACCENT));
                ...
                ui.horizontal_wrapped(|ui| {
                    ...
                    if ui
                        .add_enabled(!self.running(), egui::Button::new(language.tr("reset")))
                        .clicked()
                    {
                        self.reset();
                    }
